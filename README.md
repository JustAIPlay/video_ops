...
*   **发布排期规划 (Schedule Planning)**：
    *   **智能筛选逻辑 (按分组策略)**：
        *   **通用规则 (适用于所有分组)**：
            *   **基础门槛**：自动筛选浏览次数 ≥ 1000 的优质视频。
            *   **疲劳度控制**：确保同一视频编号在同组内的**历史发布次数** < 3 次，避免过度营销。
            *   **实时计算**：动态计算每个视频所属账号在**今日**的已发布数量。
        *   **分组策略 A (绿植、图书)**：
            *   **自动去重**：当统计数据中出现多条相同视频（分组+视频编号一致）时，系统自动保留**浏览量最高**的一条。
        *   **分组策略 B (汽车、家清、百货)**：
            *   **不去重**：保留所有符合条件的重复视频，即使视频编号相同也全部展示。
    *   **可视化排期表**：
...
定义即刻账号数据流向哪个飞书表格。
*   **视频号用户/分组名**: 必须与 API 返回的 `username` 完全一致（例如：`运营大号_A`）或与排期分组名一致（例如：`绿植`、`图书`、`汽车`、`家清`、`百货`）。
